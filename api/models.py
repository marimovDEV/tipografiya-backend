...
class User(AbstractUser):
    ...
    assigned_stages = models.JSONField(default=list, blank=True, help_text="List of stages this worker can perform")
    ...
    daily_target = models.IntegerField(default=1000, help_text="Kunlik reja (mahsulot soni)")
    quality_rating = models.DecimalField(max_digits=3, decimal_places=1, default=5.0)
...
class Transaction(models.Model):
    ...
    order_link = models.ForeignKey('Order', on_delete=models.SET_NULL, null=True, blank=True, related_name='order_transactions')
    
    # Linked Worker (for salary payments)
    worker = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='worker_transactions')
...
class WasteMaterial(models.Model):
    material = models.ForeignKey(Material, on_delete=models.CASCADE, related_name='waste_entries')
    quantity = models.DecimalField(max_digits=20, decimal_places=2)
    reason = models.TextField()
    date = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Waste: {self.material.name} ({self.quantity} {self.material.unit})"
...
class TemplateStage(models.Model):
    template = models.ForeignKey(ProductionTemplate, on_delete=models.CASCADE, related_name='stages')
...
class Order(models.Model):
    ...
    PAYMENT_STATUS_CHOICES = (
        ('unpaid', 'To\'lanmagan'),
        ('partially_paid', 'Avans olingan'),
        ('fully_paid', 'To\'liq to\'langan'),
    )
    
    client = models.ForeignKey(Client, on_delete=models.CASCADE, related_name='orders')
    template = models.ForeignKey(ProductionTemplate, on_delete=models.SET_NULL, null=True, blank=True, related_name='orders')
    ...
    # Kitob (Book) specific fields - PrintERP TZ
    book_name = models.CharField(max_length=255, blank=True, null=True, help_text="Kitob nomi")
    page_count = models.IntegerField(blank=True, null=True, help_text="Betlar soni")
    cover_type = models.CharField(max_length=100, blank=True, null=True, help_text="Muqova turi")
    binding_type = models.CharField(max_length=100, blank=True, null=True, help_text="Bog'lash turi")
    paper_weight = models.IntegerField(blank=True, null=True, help_text="Qog'oz grammaji")
    cover_weight = models.IntegerField(blank=True, null=True, help_text="Muqova grammaji")
    lamination = models.CharField(max_length=100, blank=True, null=True, help_text="Laminatsiya")
    format = models.CharField(max_length=100, blank=True, null=True, help_text="Format (A5, A4, etc.)")
    ...
    initial_payment_method = models.CharField(
        ...
        default='cash',
        help_text="Avans qanday to'langani"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.order_number:
            ...
            last_order = Order.objects.filter(
                order_number__contains=f"ORD-{date_str}"
...
class ProductionStep(models.Model):
    ...
    class Meta:
        ordering = ['order', 'sequence', 'created_at']

    def get_step_display(self):
        """Fallback for DRF serializers since we removed choices"""
        return self.step
...
class Attendance(models.Model):
    """
    Attendance records for work sessions (Clock-In / Clock-Out)
    """
    STATUS_CHOICES = (
        ('working', 'Ishlamoqda'),
        ('on_break', 'Tanaffusda'),
        ('finished', 'Ishni tugatgan'),
    )
...
class MonthlyPlan(models.Model):
    """
    Monthly income plan for the company.
    Set by super admin to track revenue targets.
    """
    ...
    plan_amount = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        help_text="Rejalashtirilgan summa"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.month}-{self.year} Rejasi: {self.plan_amount}"
...
class ProductTemplate(BaseModel):
    ...
    # Book/Magazine specific fields
    page_count = models.IntegerField(null=True, blank=True, help_text="Sahifalar soni")
    format = models.CharField(max_length=50, null=True, blank=True, help_text="Format (masalan: A4, A5)")
    binding_type = models.CharField(max_length=100, null=True, blank=True, help_text="Bog'lash turi (Yelim, Skoba, va h.k.)")
    paper_type = models.CharField(max_length=100, null=True, blank=True, help_text="Qog'oz turi")
    paper_weight = models.IntegerField(null=True, blank=True, help_text="Ichki qog'oz grammaji (gsm)")
    cover_weight = models.IntegerField(null=True, blank=True, help_text="Muqova qog'oz grammaji (gsm)")
...
class ProductionLog(models.Model):
    """
    History of quantity reporting for each production step.
    """
    production_step = models.ForeignKey(
        'ProductionStep',
        on_delete=models.CASCADE,
    ...
    def __str__(self):
        return f"{self.worker.username} - {self.production_step.step}: +{self.produced_qty}"
